...
class Mesadejogo {
  ...
  marcarbolhas(): void {
    for(let bolha of this.bolhas)
    if(bolha.y + Bolha.diametro >= height){
      bolha.vitalidade = false;
      this.erros++;
  ...
  adcBolhas():void{
    ...
    let letra = random (["a","b","c","d","e","f","g","h","i","j","k", "l","m","n","o","p","q","q","r","s","t","u","v","w","x","y","z"])
    let velocidade = random(1,10);
...
class Jogo {
  ...
  jogando(): void {
    ...
    if (this.mesadejogo.erros > 10 )
    this.ativaJogo = this.fimdejogo
    if(this.mesadejogo.acertos > 10)
      this.tempodebolhas = 20
  }
...
function keyPressed(){
  ...
  jogo.mesadejogo.removeByHit(keyCode);
