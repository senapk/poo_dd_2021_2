...
class PulaPula {
    ...
    sairDoPulaPula(nome: string) {
        ...
        let criancabusca = this.criancasNoPulaPula.find(busca => busca.getNome() === nome);

        if(criancabusca != undefined){
            this.criancasNoPulaPula = this.criancasNoPulaPula.filter(remove => remove.getNome() != criancabusca.getNome()); // remove do pula pula
    
            this.criancasNaFila.push(criancabusca); // e coloca a crianca no final da fila
        }
    }
    ...
    entrarNoPulaPula() {
        if(this.criancasNaFila && this.criancasNaFila.length > 0 ||this.criancasNaFila && this.criancasNaFila.length != undefined) {
            ...
            this.criancasNoPulaPula.push(criancabusca);
        }
    }
