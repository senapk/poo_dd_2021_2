...
class PulaPula {
    ...
    sairDoPulaPula(nome: string) {
        ...
        let crianca: Crianca | undefined = undefined;

        this.criancasNoPulaPula.forEach(c => {
            if(c.getNome() === nome)
                crianca = c;
        });
        ...
        if(crianca != undefined){
            this.criancasNoPulaPula = this.criancasNoPulaPula
                .map(c => c.getNome() != crianca.getNome()).filter(c => c.getNome() != crianca.getNome()); // remove do pula pula
    ...
    entrarNoPulaPula() {
        if(this.criancasNaFila.length > 0 && this.criancasNaFila != undefined) {
