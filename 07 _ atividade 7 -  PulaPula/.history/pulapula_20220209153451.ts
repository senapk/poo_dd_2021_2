...
class PulaPula {
    ...
    sairDoPulaPula(nome: string) {
        ...
        let crianca = this.criancasNoPulaPula.find(busca => busca.getNome() === nome);

        if(crianca != undefined){
            this.criancasNoPulaPula = this.criancasNoPulaPula
                .filter(remove => remove.getNome() != crianca.getNome()); // remove do pula pula
    ...
    entrarNoPulaPula() {
        if(this.criancasNaFila && this.criancasNaFila.length > 0 and  != undefined) {
