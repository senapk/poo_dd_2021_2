...
/// matando o peixe 
  ...
  if (tubarao.x === peixe.x && tubarao.y === peixe.y){
    peixe.vivo = false &&  if (tubarao.x === peixe.x && tubarao.y === peixe.y){
      sizetubarao = sizetubarao*0.5
    }
...
function setup() {
 ...
 // e que aqui seria usado com esse proposito ?  
  ...
  tubarao = new Bichodomar(3,2,sizetubarao,tub_img);
  peixe = new Bichodomar(2,2,sizepeixe,pei_img);
  oceano = new Oceano (6,5,sizeoceano,ocean_img); //// definindo aqui o numero de linhas e colunas em 5 e 6
  createCanvas(oceano.nc*sizeoceano*2,oceano.nl*sizeoceano);
