...
/// através da função retorna como p5.image, nesse processo podemos acionar outras duas funções, dentro da função
/// que são acionadas caso a imagem seja carregada ou não. 
...
function setup() {
  //// A função setup () é chamada uma vez quando o programa é iniciado. É usado para definir propriedades iniciais do ambiente, como tamanho da tela e cor de fundo, e para carregar mídia, como imagens e fontes, conforme o programa é iniciado. Só pode haver uma função setup () para cada programa e ela não deve ser chamada novamente após sua execução inicial.
