...
// o mesmo para o numero de linhas 
  ...
  draw(): void {
    ...
    for ( let x = 0; x < this.nc; x++){ /// esse numero de linhas e colunas será definido lá no setup quando for criado com o new o objeto
      for ( let y = 0; y < this.nl; y++){ /// a estrutura do for é instrução inicial executa apenas uma vez + condicao para execucção da terceira instrução e terceira instrunção 
        noFill();
        stroke(0)
        strokeWeight(2)
...
function setup() {
 ...
 // e que aqui seria usado com esse proposito ?  
  ...
  peixe = new Bichodomar(1,1,size,pei_img);
  oceano = new Oceano (5,6,size,ocean_img); //// definindo aqui o numero de linhas e colunas em 5 e 6
