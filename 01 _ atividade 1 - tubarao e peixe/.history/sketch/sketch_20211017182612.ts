...
class Bichodomar {
  ...
  draw(): void {
    image(this.tub_img, this.x*this.nado, this.y*this.nado, this.nado,this.nado);
    /// posição de x,y,largura e altura, aqui como preciso que o peixe seja menor 
    /// que o tubarão, e partindo de quem as imagens já tem tamanhos diferentes eu resolvi dividir a altura por 2
    /// deixando o peixe menor e o tubarão maior, fica então 100 de largura e 50 de altura 
  }
}
...
function setup() {
  ...
  let size = 100
  tubarao = new Bichodomar(2,2,size,tub_img);
  peixe = new Bichodomar(1,1,size/2,pei_img);
