...
class Bichodomar {
  ...
  draw(): void { 
    image(this.img, this.x*this.nado, this.y*this.nado, this.nado,this.nado/2);
...
function setup() {
 ...
 // e que aqui seria usado com esse proposito ?  
  ...
  let xpei = 0
  let ypei = 0 
  tubarao = new Bichodomar(xtub,ytub,size*2,tub_img);
  tubaraogordo = new Bichodomar(2,2,size*4,tub_img);
  peixe = new Bichodomar(xpei,ypei,size,pei_img);
