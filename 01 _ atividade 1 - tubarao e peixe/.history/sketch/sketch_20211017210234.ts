...
// o mesmo para o numero de linhas 
  ...
  draw(): void {
    ...
    for ( let x = 0; x < this.nc; x++){ /// esse numero de linhas e colunas será definido lá no setup quando for criado com o new o objeto
      for ( let y = 0; y < this.nl; y++){ /// a estrutura do for é instrução inicial executa apenas uma vez + condicao para execucção da terceira instrução e terceira instrumção
        noFill();
        stroke(0)
        strokeWeight(2)
